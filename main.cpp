...
ID3D12Resource* CreateTextureResource(ID3D12Device* device, const DirectX::TexMetadata& metadata) {
	...
	resourceDesc.SampleDesc.Count = 1;//駿府リングカウント。1固定。
	resourceDesc.Dimension = D3D12_RESOURCE_DIMENSION(metadata.dimension);//Textureの次元数。普段使っているのは2次元
	//2.利用するHeapの設定
	D3D12_HEAP_PROPERTIES heapProperties{};
	heapProperties.Type = D3D12_HEAP_TYPE_CUSTOM;//駒かい設定を行う
...
void UploadTextureData(ID3D12Resource* texture, const DirectX::ScratchImage& mipImages) {
	...
	for (size_t mipLevel = 0; mipLevel < metadata.mipLevels; ++mipLevel) {
		...
		const DirectX::Image* img = mipImages.GetImage(mipLevel, 0, 0);//Textureに転送
...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
	...
	if (SUCCEEDED(device->QueryInterface(IID_PPV_ARGS(&infoQueue)))) {
		...
		D3D12_MESSAGE_ID denyIds[] = {
			//Windows11でのDXGIデバッグレイヤーとDX12デバッグレイヤーの相互作用バグに夜エラーメッセージ
	...
	//曲面の幅。ウインドウのクライアント領域を同じものにしておく
	swapChainDesc.Width = kClientWidth;
	//曲面の高さ。ウインドウのクライアント領域を同じものにしておく
	...
	*wvpData = MakeIdenttity4x4();
	...
	IDxcBlob* pixelShadeBlob = CompileShader(L"Object3D.PS.hlsl",
		L"ps_6_0", dxcUtils, dxcCompiler, includeHandler);
	assert(pixelShadeBlob != nullptr);
	...
	graphicsPipelineStateDesc.PS = { pixelShadeBlob->GetBufferPointer(),
	pixelShadeBlob->GetBufferSize() };//PixelShader
	...
	*transformationMatrixDataSprite = worldViewProjectionMatrixSprite;
	...
	commandList->RSSetViewports(1, &viewport);//Viewpostを設定
	commandList->RSSetScissorRects(1, &scissorRect);//Scirssorの設定
	...
	//SRVのDescriptorTableの先頭を設定。2はrootParameter[2」dearu.
	...
	while (msg.message != WM_QUIT) {
		...
		if (PeekMessage(&msg, NULL, 0, 0, PM_REMOVE)) {
			...
			*wvpData = worldMatrix;
			...
			Matrix4x4 projectionMatrix = MakePerspecriveFovMatrix(0.45f,
			...
			//Fenceの値を更新
			fenceValue++;
			//GPUがここまでたどりついた時に、Fenceの値を代入するようにSignalを送る
			commandQueue->Signal(fence, fenceValue);

			//Fenceの値が指定したSignal部にたどりついているか確認する
			//GetCompletedValueの初期値はFence作成時に渡した初期値
			if (fence->GetCompletedValue() < fenceValue) {
				//指定したSignalにたどりついていないので、たどりつくまで持つようにイベントを設定する
